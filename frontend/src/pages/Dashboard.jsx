...
// Safely resolve the HighchartsReact component for Vite
const HighchartsReact = HighchartsReactWrapper.default || HighchartsReactWrapper;

// Safely initialize the Bubble Chart module
...
import { 
  ...
  Award, Briefcase, Package, FileText, ShoppingCart, Menu, Sun, Moon, Trophy 
} from 'lucide-react';
...
export default function Dashboard() {
      ...
      .then(response => {
        ...
        setBubbleOptions({
            ...
            tooltip: {
                ...
                backgroundColor: isDarkMode ? '#1f2937' : '#ffffff',
                style: { color: isDarkMode ? '#ffffff' : '#000000' },
                headerFormat: '<table style="font-size: 13px">',
            ...
            plotOptions: {
                bubble: {
                    ...
                    dataLabels: { 
                      ...
                      allowOverlap: false // Prevents the names from becoming an unreadable mess
                    }
                }
            },
            series: [{ 
              ...
              data: bubbleData, 
              colorByPoint: true, 
              marker: { fillOpacity: 0.6, lineWidth: 1, lineColor: isDarkMode ? '#1f2937' : '#ffffff' } 
            }],
  ...
  const [sankeyOptions, setSankeyOptions] = useState(null); // <-- NEW STATE
      ...
      .then(response => {
        ...
        setSankeyOptions({
          ...
          tooltip: {
            backgroundColor: isDarkMode ? '#1f2937' : '#ffffff',
            style: { color: isDarkMode ? '#ffffff' : '#000000' },
            // Formats the Blocks (Nodes)
            nodeFormat: '{point.name}: <b>{point.sum:,.2f} MAD</b><br/>',
            // Formats the Lines (Links)
        ...
        <main className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 lg:col-span-2 transition-colors duration-300">
              ...
              <div className="h-96">
                {chartOptions ? <HighchartsReact highcharts={Highcharts} options={chartOptions} containerProps={{ style: { height: '100%' } }} /> : <div className="h-full flex items-center justify-center text-gray-400">Chargement...</div>}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col transition-colors duration-300">
              ...
              <div className="h-96 flex-1">
                {pieOptions ? <HighchartsReact highcharts={Highcharts} options={pieOptions} containerProps={{ style: { height: '100%' } }} /> : <div className="h-full flex items-center justify-center text-gray-400">Chargement...</div>}
              </div>
            </div>
          </div>
              ...
              <div className="overflow-auto max-h-[380px] pr-2 custom-scrollbar flex-1">
                <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead className="sticky top-0 bg-white dark:bg-gray-900 z-10 shadow-sm">
                    <tr>
                      <th className="pb-4 font-semibold text-sm text-gray-400 border-b border-gray-100 dark:border-gray-800 w-12 text-center">#</th>
                      <th className="pb-4 font-semibold text-sm text-gray-400 border-b border-gray-100 dark:border-gray-800 w-1/4">Code</th>
                      <th className="pb-4 font-semibold text-sm text-gray-400 border-b border-gray-100 dark:border-gray-800 w-1/3">Raison Sociale</th>
                      <th className="pb-4 font-semibold text-sm text-gray-400 border-b border-gray-100 dark:border-gray-800 text-center">Cmds</th>
                      <th className="pb-4 font-semibold text-sm text-gray-400 border-b border-gray-100 dark:border-gray-800 text-right">CA</th>
                      <th className="pb-4 font-semibold text-sm text-gray-400 border-b border-gray-100 dark:border-gray-800 text-center">Statut</th>
                    </tr>
                  </thead>
                  <tbody>
                    {topClients.length > 0 ? (
                      topClients.map((client, index) => (
                        <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                          <td className="py-4 border-b border-gray-50 dark:border-gray-800/50 text-sm font-bold text-gray-400 text-center">{index + 1}</td>
                    ...
                    <div key={index} className="flex flex-col gap-2">
                      <div className="flex justify-between items-center gap-4">
                        <div className="flex items-center gap-3 flex-1">
                          <div className={`w-8 h-8 flex-shrink-0 rounded-full flex items-center justify-center font-bold text-xs shadow-sm ${index === 0 ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30' : index === 1 ? 'bg-gray-200 text-gray-600 dark:bg-gray-700' : index === 2 ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/30' : 'bg-blue-50 text-blue-600 dark:bg-blue-900/20'}`}>
                            #{index + 1}
                          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors duration-300">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">CA: Année En Cours vs Précédente</h3>
              <div className="h-96">
            ...
            <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors duration-300 flex flex-col">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Qualité du Portefeuille</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">Taille bulle = Vol. Commandes</p>
              {/* FIX 1: Changed h-80 to h-96 and added flex-1 to stretch the chart */}
              <div className="h-96 flex-1">
                {bubbleOptions ? <HighchartsReact highcharts={Highcharts} options={bubbleOptions} containerProps={{ style: { height: '100%' } }} /> : <div className="h-full flex items-center justify-center text-gray-400">Chargement...</div>}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors duration-300 mb-8">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Flux de Revenus : Division → Commercial → Client</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-6">Épaisseur du flux = Chiffre d'Affaires</p>
            <div className="h-[500px]">
              {sankeyOptions ? <HighchartsReact highcharts={Highcharts} options={sankeyOptions} containerProps={{ style: { height: '100%' } }} /> : <div className="h-full flex items-center justify-center text-gray-400">Chargement...</div>}
            </div>
          </div>
